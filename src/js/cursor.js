...
export class GooCursor {
  ...
  constructor(DOM_el) {
    ...
    mousepos.x = this.DOM.el.clientWidth / 2;
    mousepos.y = this.DOM.el.clientHeight / 2;

    // Cells wrapper element that gets the SVG filter applied
    this.DOM.inner = this.DOM.el.querySelector(".celled-block");
  ...
  layout() {
    ...
    for (let i = 0; i < this.cellsTotal; ++i) {
      innerStr +=
        customColorsTotal === 0
          ? '<div class="celled-block__item"></div>'
